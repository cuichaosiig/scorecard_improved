...
def pow_kernel_func(df,topcols,yname,lambda_list=[],dovalid=1):
    '''
    #批量进行指标数学转换
    df: 进行转化的dataframe 
    topcols: 需要进行数学转换的指标集合
    ...
    Author: 崔超
    '''
    funclist = {'pow_'+j:f"lambda df: df['{j}']*df['{j}']" \
                    for j in topcols \
			}

    if dovalid==1:
        lambda_list = get_validtrans(df,yname,funclist,{'power_kernel':-1},lambda_list)
    else:
        lambda_list = transferfunclist(funclist,pardict,lambda_list)
